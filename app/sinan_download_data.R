...
confirmados <- TRUE
for(i in startFile:length(myfiles)){
  tempFile <- fread(file.path(dir, "arb_tsv", myfiles[i]), stringsAsFactors = FALSE, showProgress = FALSE)
  tempFile$weekStart <- NA
  myYearCalen <- paste0(20,str_replace_all(myfiles[i], "[^0-9]", ""))
  cale_year <- subset(calendario, ANO == myYearCalen)
  for (icale in 1:nrow(cale_year)){
...
if (confirmados == TRUE){
  write.table(baseFinal, file.path(dir, "/2015-2025_CHIKV_CONFIRMADOS_new.tsv"), sep = "\t", row.names = FALSE)
}else{
...
if (confirmados == TRUE){
  write.table(baseFinal, file.path(dir, "/2016-2025_ZIKV_CONFIRMADOS_new.tsv"), sep = "\t", row.names = FALSE)
}else{
  write.table(baseFinal, file.path(dir, "/2016-2025_ZIKV_NOTIFICADOS_new.tsv"), sep = "\t", row.names = FALSE)
}
...
write.table(arbo_agre, file.path(dir, "/2014-2025_ARBO_CONFIRMADOS_new.tsv"), sep = "\t", row.names = FALSE)
